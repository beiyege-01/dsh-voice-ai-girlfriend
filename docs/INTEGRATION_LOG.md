...
- **一键脚本 start-dsh-voice.cmd 启动后全功能自检**:端口 3080/8765/3000/6099 全部监听;STT(真实人声识别正常)、TTS(合成正常)、QQ 发送(文本送达主号)全部 ✅。
- **多标签页语音叠加问题**:确认根因=浏览器开了多个 DSH 标签页(每标签页独立 AudioContext 朗读器,各自播放叠加)。日志 10+ 个 /api/qq/ws 连接为铁证。关到单标签页后恢复正常。**遗留建议**:插件可加同源标签页互斥(只允许一个播放语音)防复发。
- **"桥接没跑导致 STT/TTS 失效"**:手动只起 DSH web(3080)时桥接(8765)未启动 → 语音全失效;用一键脚本三合一启动即恢复。教训:重启后必须用 start-dsh-voice.cmd 全起。

## QQ 推送开关(2026-08-16)

- **需求**:电脑上操作时回复不停推 QQ 很烦,要一个按钮主动关闭/开启推送。
